...
(() => {
  ...
  function normalizePageHead(head){
    ...
    const tools=ensureTools(head,block);
    const actions=ensureActions(head);

    // share / print always top right
    candidates.filter(el=>['share','print'].includes(roleOf(el))).forEach(el=>{
      normalizeIconButton(el,roleOf(el));
      if(el.parentElement!==tools) tools.appendChild(el);
      el.dataset.jk33Done='tool';
    });
    ...
    qa('.jk33-title-tools,.jk33-segment-scroll',head).forEach(old=>{
      if(old!==tools && old!==actions && !qa('button,a,input,select,textarea',old).some(visible)) old.remove();
    });
  }
